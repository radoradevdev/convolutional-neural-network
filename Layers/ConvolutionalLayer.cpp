...
void ConvolutionalLayer::_out_dimension() {
    ...
    _out_dim[0] = _specs[0]; // The output depth is equal to the number of kernels
    // of the filter
    _out_dim[1] = y_int;
    _out_dim[2] = x_int;
...
void ConvolutionalLayer::fwd(Elements image, Elements &out) {
  ...
  */
    ...
    for (int kernel = 0; kernel < n_kernel; kernel++) {
        ...
        for (int layer = 0; layer < depth;
             layer++) { // each kernel has n (3) layers, one for each of the n (3)
            // layers of the image, the depth.

            y_out = 0, x_out = 0;

            for (int y = 0; y < _image_dim[1] - f_y;
...
void ConvolutionalLayer::bp(Elements d_out_vol, Elements &d_input) {
    ...
    Elements d_filters(
            ...
            _specs[3]); // The list of lists of error terms (lowercase deltas)
    ...
    for (int kernel = 0; kernel < n_kernel; kernel++) {
        ...
        for (int y = 0; y < _image_dim[1] - f_y - 2 * _padding;
             y += _stride) { // image = ( depth x H x W )
